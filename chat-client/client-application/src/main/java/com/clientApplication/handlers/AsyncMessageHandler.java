...
public class AsyncMessageHandler implements MessageHandler {
    ...
    private void handleInviteReceived(List<String> parts) {
        if (parts.size() < 6) {
            logger.warn("INVITE_RECEIVED malformado: {}", parts);
            return;
        }
        ...
        List<InviteListener> listenersCopy;
        synchronized (inviteListeners) {
            listenersCopy = new ArrayList<>(inviteListeners);
        }
        for (InviteListener l : listenersCopy) {
            try {
                l.onInvite(event);
            } catch (Exception e) {
                logger.error("Error notificando InviteListener", e);
            }
        }
    }
    ...
    private void handleChannelMessage(List<String> parts) {
        ...
        for (ChannelMessageListener l : listenersCopy) {
            try {
                ...
                logger.error("Error notificando ChannelMessageListener", e);
            }
        }
    }
    ...
    private void handleChannelAudio(List<String> parts) {
        ...
        try {
            ...
            logger.error("Audio base64 inválido de canal {}: {}", channelId, e.getMessage());
            return;
        }
        ...
        synchronized (channelAudioListeners) {
            listenersCopy = new ArrayList<>(channelAudioListeners);
        }
        for (ChannelAudioListener l : listenersCopy) {
            try {
                l.onChannelAudio(event);
            } catch (Exception e) {
                logger.error("Error notificando ChannelAudioListener", e);
            }
        }
    }
    ...
    public void registerChannelAudioListener(ChannelAudioListener listener) {
        if (listener != null) {
            synchronized (channelAudioListeners) {
                channelAudioListeners.add(listener);
            }
        }
    }

    public void registerInviteListener(InviteListener listener) {
        if (listener != null) {
            synchronized (inviteListeners) {
