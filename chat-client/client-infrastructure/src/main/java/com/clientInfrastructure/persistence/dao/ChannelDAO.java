...
public class ChannelDAO {
    ...
    public void upsertChannel(ChannelDTO channel) {
        ...
        try (Connection conn = connectionManager.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            stmt.setTimestamp(5, Timestamp.valueOf(channel.getCreatedAt() != null ? channel.getCreatedAt() : LocalDateTime.now()));
            stmt.executeUpdate();
        } catch (SQLException e) { logger.error("Error upsert canal: {}", e.getMessage()); }
    }
    ...
    public List<ChannelDTO> listChannels() {
        ...
        } catch (SQLException e) { logger.error("Error listando canales: {}", e.getMessage()); }
        return list;
    }
}
